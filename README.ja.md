...
- **上 pane** — セッションを最終更新の新しい順に並べたリスト。列は
  ...
  実行中と確認できた `claude` プロセスがある場合のみ表示)、`CWD`
  (basename)。この pane の最下部には使用可能なキーの説明を表示します。
- **下 pane** — カーソルで選択中のセッションのプレビュー。ディレクトリ、
  (Claude Code が生成していれば)タイトル、ファイルサイズ、開始/終了時刻、
  そのセッションで送った直近のプロンプトを表示します。
...
コマンドや diff、ファイル内容は syntax highlight されます。コマンドの実行
